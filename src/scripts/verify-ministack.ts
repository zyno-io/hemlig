...
const run = async (): Promise<void> => {
  ...
  const keyResponse = await kms.send(
    new CreateKeyCommand({ Description: "MiniStack test key" }),
  );
  ...
  if (existingObject.versionId !== object.versionId) {
    throw new Error(
      "Immutable object retry did not return the existing version.",
    );
  }
  const stored = await objects.getJson<typeof revision>(
    bucket,
    object.key,
    object.versionId,
  );
  ...
  const catalog = await repository.listSecrets("local", "testing", {
    owner: "platform",
  });
  if (
  ...
  if (consumers.consumers[0]?.consumerId !== "local-east") {
    throw new Error(
      "Consumer directory index did not return the expected consumer.",
    );
  }
  ...
  if (!deleteDenied) {
    throw new Error(
      "MiniStack allowed deletion of a Compliance-retained object.",
    );
  }
