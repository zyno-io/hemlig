...
const buildEvent = (
    ...
    requestContext: {
        ...
        authorizer: {
            jwt: {
                claims: { iss: config.adminJwtIssuer, aud: config.adminJwtAudience, sub: 'admin-1' },
            },
        },
    },
...
describe('POST /v1/admin/issuer', () => {
    ...
    beforeEach(() => {
        ...
        const kms = {
            send: jest.fn(async (command: unknown) => {
                if (command instanceof GenerateDataKeyCommand) {
                    return { Plaintext: Buffer.alloc(32, 9), CiphertextBlob: Buffer.from('wrapped-data-key') };
                }
        ...
        fakeApp = {
            ...
            audit: {
                write: jest.fn(async (event: Record<string, unknown>) => ({ eventId: 'evt-1', at: '2026-08-23T00:00:00.000Z', ...event })),
        ...
        withErrorResponse.mockImplementation(async (
            event: APIGatewayProxyEventV2,
            action: (app: Application, correlationId: string, setAuditContext: (context: unknown) => void) => Promise<APIGatewayProxyStructuredResultV2>,
        ) => {
            try {
    ...
    it('creates the issuing root and returns 201 without leaking the encrypted key', async () => {
        const response = await handler(buildEvent('POST', '/v1/admin/issuer', { 'idempotency-key': 'first-request-key' }));
    ...
    it('is idempotent: a second call returns 200 for the already-created root and still never leaks the key', async () => {
        const first = await handler(buildEvent('POST', '/v1/admin/issuer', { 'idempotency-key': 'first-request-key' }));
        const second = await handler(buildEvent('POST', '/v1/admin/issuer', { 'idempotency-key': 'second-request-key' }));

        expect(second.statusCode).toBe(200);
        const firstBody = JSON.parse(first.body as string) as Record<string, unknown>;
        const secondBody = JSON.parse(second.body as string) as Record<string, unknown>;
...
describe('GET /v1/admin/secrets', () => {
    ...
    beforeEach(() => {
        listSecrets = jest.fn(async () => ({ secrets: [], nextCursor: JSON.stringify({ pk: 'SECRET#x', sk: 'HEAD' }) }));
        ...
        const repository = { listSecrets, searchSecrets } as unknown as DynamoRepository;
        fakeApp = {
            config,
            repository,
            objects: {} as unknown as ObjectStore,
    ...
    it('returns a bounded, complete-or-truncated page with no nextCursor when q is present', async () => {
        const event = buildEvent('GET', '/v1/admin/secrets', {}, { environment: 'test', pathPrefix: 'payments', q: 'stripe' });

        const response = await handler(event);

        expect(searchSecrets).toHaveBeenCalledWith('test', 'payments', {}, 'stripe');
    ...
    it('keeps the existing cursor-paginated behavior unchanged when q is absent', async () => {
        const event = buildEvent('GET', '/v1/admin/secrets', {}, { environment: 'test' });
    ...
    it('lists administrator-defined environments and creates a new one', async () => {
        ...
        const listResponse = await handler(buildEvent('GET', '/v1/admin/environments'));
        const createEvent = {
        ...
        expect(JSON.parse(createResponse.body as string)).toMatchObject({ name: 'staging' });
    });
});
...
describe('/v1/admin/folders', () => {
    ...
    beforeEach(() => {
        createFolder = jest.fn(async (input: { environment: string; path: unknown; actor: unknown }) => ({
        ...
        fakeApp = {
            ...
            cursors: {} as unknown as CursorCodec,
            environments: {} as unknown as EnvironmentService,
            folders: { create: createFolder, remove: removeFolder } as unknown as FolderService,
            secrets: {} as unknown as SecretService,
            consumers: {} as unknown as ConsumerService,
            clients: {} as unknown as AwsClients,
        };
    ...
    it('returns 409 when the folder already exists or the registry is full', async () => {
        createFolder.mockRejectedValueOnce(conflict('The folder already exists or the registry is full.'));
        const event = {
            ...buildEvent('POST', '/v1/admin/folders'),
        ...
        expect(JSON.parse(response.body as string)).toMatchObject({ error: { code: 'conflict' } });
    });

    it('rejects an unknown environment the same way other scoped routes do', async () => {
        createFolder.mockRejectedValueOnce(notFound('The requested environment is not configured.'));
        const event = {
            ...buildEvent('POST', '/v1/admin/folders'),
        ...
        expect(JSON.parse(response.body as string)).toMatchObject({ error: { code: 'not_found' } });
    });

    it('rejects an invalid path', async () => {
        createFolder.mockRejectedValueOnce(badRequest('path must be a lowercase slash-delimited path of at most 256 characters.'));
        const event = {
            ...buildEvent('POST', '/v1/admin/folders'),
            body: JSON.stringify({ environment: 'prod', path: 'Payments' }),
        };
        ...
        expect(JSON.parse(response.body as string)).toMatchObject({ error: { code: 'bad_request' } });
    });

    it('deletes an empty folder and returns 204', async () => {
        const event = buildEvent('DELETE', '/v1/admin/folders', {}, { environment: 'prod', path: 'payments/adyen' });

        const response = await handler(event);

        expect(removeFolder).toHaveBeenCalledWith({ environment: 'prod', path: 'payments/adyen' });
        expect(response.statusCode).toBe(204);
    });

    it('returns 409 when the folder is not empty', async () => {
        removeFolder.mockRejectedValueOnce(conflict('The folder is not empty: a secret exists at this path or nested beneath it.'));
        const event = buildEvent('DELETE', '/v1/admin/folders', {}, { environment: 'prod', path: 'payments' });
    ...
    it('returns 404 when the path is only a derived folder, not a record', async () => {
        removeFolder.mockRejectedValueOnce(notFound('No folder record exists at this path.'));
        const event = buildEvent('DELETE', '/v1/admin/folders', {}, { environment: 'prod', path: 'payments' });
