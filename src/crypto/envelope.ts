...
export interface PayloadBinding {
  ...
  /** Absent only when decrypting an immutable pre-UID revision. */
...
export class EnvelopeCrypto {
  ...
  public async decrypt(revision: PayloadRevision): Promise<SecretPayload> {
    const binding: PayloadBinding = {
      ...
      // Revisions written before UID migration remain immutable. Their
      // KMS/AAD binding used the public ID, so retain that shape.
      secretUid: revision.secretUid,
      secretId: revision.secretId,
      payloadVersionId: revision.payloadVersionId,
    };
    const encryptedDataKey = Buffer.from(
      revision.payload.encryptedDataKey,
      "base64",
    );
    ...
    try {
      const decipher = createDecipheriv(
        "aes-256-gcm",
        key,
        Buffer.from(revision.payload.iv, "base64"),
      );
