...
describe('EnvelopeCrypto', () => {
    it('binds an AES-GCM payload to its exact payload version', async () => {
        ...
        const kms = {
            send: jest.fn()
                .mockResolvedValueOnce({ Plaintext: key, CiphertextBlob: Buffer.from('encrypted-key') })
        ...
        const revision = await crypto.encrypt(
            ...
            { environment: 'staging', secretId: 'sec-example', payloadVersionId: 'pay-example' },
            { type: 'human', id: 'operator-1' },
            '2026-08-22T00:00:00.000Z',
        );
        const payload = await crypto.decrypt(revision);
        expect(payload).toEqual({ PASSWORD: { encoding: 'utf8', value: 'correct horse battery staple' } });
    ...
    it('rejects a ciphertext rebound to a different payload version', async () => {
        ...
        const revision = await crypto.encrypt(
            ...
            { environment: 'production', secretId: 'sec-example', payloadVersionId: 'pay-one' },
            { type: 'human', id: 'operator-1' },
            '2026-08-22T00:00:00.000Z',
        );
