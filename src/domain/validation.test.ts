...
describe("secret validation", () => {
  ...
  it("uses secret IDs, not metadata paths, for hierarchy and agent scope", () => {
    expect(
      parseMetadata({
    ...
    expect(() => parseMetadata({ path: "payments/stripe/production" })).toThrow(
      "metadata.path",
    );
  ...
  it("accepts slash-separated secret IDs but rejects empty path segments", () => {
    expect(() =>
      assertSecretIdentifier("payments/stripe/api-key", "secretId"),
    ).not.toThrow();
    expect(() =>
      assertSecretIdentifier("/payments/api-key", "secretId"),
    ).toThrow("leading");
    expect(() =>
      assertSecretIdentifier("payments/api-key/", "secretId"),
    ).toThrow("trailing");
  ...
  it("uses the same secret-ID grammar for catalog prefixes", () => {
    ...
    expect(() => parseCatalogPathPrefix("payments//stripe")).toThrow(
      "pathPrefix",
    );
  });

  it("rejects ambiguous organizational tags", () => {
    expect(() => parseMetadata({ tags: { Owner: "payments" } })).toThrow(
      "metadata.tags",
    );
    expect(() =>
      parseCatalogTagFilters("owner:payments,owner:platform"),
    ).toThrow("unique");
  });
});
