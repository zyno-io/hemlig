...
interface FakeApi {
  ...
  getSecretPayload?: (secretId: string) => Promise<{
    secretId: string;
...
const buildRouter = (): Router =>
  createRouter({
    ...
    routes: [
      ...
      { path: "/e/:env/secrets/:secretId/metadata", name: "secret-metadata", component: { template: "<div/>" } },
      { path: "/e/:env/secrets/:secretId/revisions", name: "secret-revisions", component: { template: "<div/>" } },
      { path: "/e/:env/secrets/:secretId/payload", name: "secret-payload", component: { template: "<div/>" } },
    ],
  });
...
const mountView = async (api: FakeApi): Promise<{ wrapper: ReturnType<typeof mount> }> => {
  ...
  await router.push({ name: "secret", params: { env: "dev", secretId: "stripe-api-key" } });
...
describe("SecretDetail path", () => {
  it("links the path to its folder in the browse tree", async () => {
    const { wrapper } = await mountView({
      getSecret: async () => secretFixture({ metadata: { path: "payments/stripe" } }),
    });

    const link = wrapper.findAll("a").find((a) => a.text() === "payments/stripe");
    expect(link).toBeDefined();
    expect(link?.attributes("href")).toBe("/e/dev/secrets/browse/payments/stripe");
  });

  it("says a rootless secret is at the root rather than showing a dash with no route", async () => {
    const { wrapper } = await mountView({ getSecret: async () => secretFixture({ metadata: {} }) });
  ...
  it("offers a Move affordance to the metadata editor", async () => {
    ...
    expect(move?.attributes("href")).toBe("/e/dev/secrets/stripe-api-key/metadata");
  });
  ...
  it("reveals the current payload only after the explicit audited action", async () => {
    const getSecretPayload = vi.fn(async () => ({
      ...
      payload: { PASSWORD: { encoding: "utf8" as const, value: "correct-horse" } },
    }));
    const { wrapper } = await mountView({
      getSecret: async () => secretFixture({ payloadVersionId: "pay-1", payloadKeyCount: 1 }),
      getSecretPayload,
    });

    expect(getSecretPayload).not.toHaveBeenCalled();
    expect(wrapper.text()).toContain("separately audited action");
    ...
    expect(getSecretPayload).toHaveBeenCalledWith("stripe-api-key");
    expect(wrapper.text()).toContain("correct-horse");

    const hide = wrapper.findAll("button").find((button) => button.text() === "Hide");
