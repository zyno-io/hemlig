...
const metadata = reactive<MetadataDraft>({ name: "", description: "", path: "", tags: [] });
...
const adopt = (revision: ControlRevision | undefined): void => {
  ...
  metadata.name = revision.metadata.name;
    ...
    <form v-if="data" class="space-y-4 rounded border border-line bg-surface-raised p-4" @submit.prevent="submit">
      ...
      <div class="flex items-center gap-3">
        <button
          ...
          :disabled="metadata.name.trim().length === 0 || update.phase.value.kind === 'submitting'"
          type="submit"
        >
